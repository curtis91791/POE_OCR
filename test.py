...
def get_item_text():
    ...
    #gray = cv2.medianBlur(gray, 1)  # 去除雜點
    resized_gray = cv2.resize(gray, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC) #放大 N
    _, thresh = cv2.threshold(resized_gray, 110, 255, cv2.THRESH_BINARY_INV) #調整背景光影
    text = pytesseract.image_to_string(thresh, lang=LANG, config=TESSERACT_CONFIG)
    #--------show out img--------
...
while True:
    ...
    # if len(target_keywords)==1: #complete
    #     break
    # elif len(target_keywords)==2: #崇高
    #     use_currency("ex")
    #     break
    # elif len(target_keywords)==3: #富豪
    #     use_currency("regal")
    #     time.sleep(0.2)
    #     affix_text = get_item_text()
    #     match_target(affix_text)
    #     time.sleep(0.2)
    #     if len(target_keywords)==3: #重鑄
    #         target_keywords = original_targets.copy()
    ...
    #         affix_text = get_item_text()
    #         match_target(affix_text)
    # elif re.search("混沌技能增加3%攻擊",match_target(affix_text)): #增幅
